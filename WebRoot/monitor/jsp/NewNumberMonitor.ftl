		...
		<div style="display: none;">
			<div id="explainDialog">
				<table>
					...
					<tr class="color">
						<td>新数派发量</td><td>数据派发时间在查询周期内所派发数据量</td><td>新数派发量</td>
					</tr>
					<tr>
						<td>新数派发金额</td><td>数据派发时间在查询周期内所派发数据的金额</td><td>新数派发金额</td>
					</tr>
					<tr class="color">
						<td>新数外呼量</td><td>数据派发时间、数据外呼时间在查询周期内所派发数据的外呼量</td><td>新数外呼量</td>
					</tr>
					<tr>
						<td>新数完成率</td><td>新数据当天拨打完成的办理</td><td>新数外呼量/新数派发量*100，保留两位小数点，小数点最后一位四舍五入。即显示格式为99.88%</td>
					</tr>
					<tr class="color">
						<td>新数接通量</td><td>数据派发时间、数据拨打时间在查询周期内所派发数据的接通量</td><td>新数接通量</td>
					</tr>
					<tr>
						<td>新数成功受理量</td><td>数据派发时间、外呼结果码保存时间在查询周期内所派发数据的成功受理量</td><td>新数成功受理量</td>
					</tr>
				</table>
			</div>
		</div>
	...
	<script>
		$(function(){
			...
			//alert(window.location.href);
				$('#data_table').datagrid({
				    url:'../dataview/getNewNumberMonitor.action',
				    queryParams:{
			...
			$('#exportBtn').click(function(){
		        $('#condition_form').form({
		            url : '../dataview/exportNewNumberMonitorExcel.action',
		            dataType: 'text',
		            queryParams:{
				        center:$('#center').combobox('getValue'),
				        group:$('#group').combobox('getValue'),
