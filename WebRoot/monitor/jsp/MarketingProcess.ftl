		...
		<div style="display: none;">
			<div id="explainDialog">
				<table>
					...
					<tr class="color">
						<td>外呼数据量</td><td>查询周期内外呼数据量</td><td>查询周期内外呼数据量</td>
					</tr>
					<tr>
						<td>成功受理量</td><td>查询周期内成功受理的数据量</td><td>查询周期内成功受理的数据量，主营成功才统计</td>
					</tr>
					<tr class="color">
						<td>成功批核金额</td><td>查询周期内成功批核的金额，如涉及批核结果数据流转则将金额统计入数据同步当天</td><td>查询周期内成功批核的金额，只统计主营</td>
					</tr>
					<tr>
						<td>接通通话总时长</td><td>接通通话总时长</td><td>通话结束时间在查询周期内外呼接通后的总通话时长（不含振铃、不含话后满意度时长）</td>
					</tr>
				</table>
			</div>
		</div>
	...
	<script>
		$(function(){
			...
			//alert(window.location.href);
				$('#data_table').datagrid({
				    url:'../dataview/getMarketingProcess.action',
				    queryParams:{
			...
			$('#exportBtn').click(function(){
		        ...
		        $('#condition_form').form({
		            url : '../dataview/exportMarketingProcessExcel.action',
		            dataType: 'text',
		            queryParams:{
				        center:$('#center').combobox('getValue'),
				        group:$('#group').combobox('getValue'),
